...
def send_mail(
    ...
    html,
    text,
    from_email="bureaucrat@debating.web.app",
...
def login():
    ...
    if not user:
        return jsonify(
            {"type": "error", "message": "That user does not exist.", "suggestion": ""}
        )
    ...
    return jsonify({"type": "data", "data": {"token": token}})
...
def register():
    ...
    if existing_user:
        return jsonify(
            {"type": "error", "message": "That user already exists.", "suggestion": ""}
        )
    ...
    if not os.environ.get("TESTING"):
        ...
        send_mail(
            user.email,
            "[d3bate] email verification",
            html,
            "Please copy the following URL and paste it into your browser: {}".format(
                jwt_token
            ),
        )
...
def resend_email_verification():
    ...
    if not os.environ.get("TESTING"):
        ...
        html = render_template(
            "email_confirmation.html",
            confirm_email="https://d3bate.herokuapp.com/auth/confirm?token={}".format(
                jwt_token
            ),
        )
        send_mail(
    ...
    if "email_confirmation" in token:
        if token["email_confirmation"] is True:
            ...
            return render_template(
                "confirm_verification.html",
                redirect_url=current_app.config["FRONTEND_URL"],
            )
    else:
        return render_template(
            "confirm_failure.html", application_url=current_app.config["FRONTEND_URL"]
        )
