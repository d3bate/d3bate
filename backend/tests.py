...
class E2E(unittest.TestCase):
    ...
    def test_application_end_to_end(self):
        ...
        create_existing_user = self.client.post(
            "/auth/register",
            json={
                "username": "teymour",
                "name": "Teymour Aldridge",
        ...
        token = self.client.post(
            "/auth/login", json={"identifier": "me@teymour.tk", "password": "hello"}
        ).json
        self.assertTrue(token["type"] == "data")
        self.token = token["data"]["token"]

        create_club = self.client.post(
            "/api/club/create",
            json={"club_name": "Debating", "school_website": "https://teymour.tk"},
            headers={"Authorization": "Bearer {}".format(self.token)},
        )
        self.assertTrue(create_club.json["type"] == "data")

        get_club_list = self.client.get(
            "/api/club/get_all",
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
        ...
        add_training_session_1 = self.client.post(
            "/api/club/training/add",
            json={
                "club_id": club_id["id"],
                "livestream": False,
        ...
        get_training_session_1 = self.client.get(
            "/api/club/training/all",
            json={"club_id": club_id["id"]},
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
        ...
        update_training_session_1 = self.client.post(
            "/api/club/training/update",
            json={
                "session_id": training_session["id"],
                "delta": {"start_time": start_time + 10},
            },
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
        self.assertTrue(update_training_session_1["type"] == "success")

        get_training_session_1_updated = self.client.get(
            "/api/club/training/all",
            json={"club_id": club_id["id"]},
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
        ...
        leave_club = self.client.post(
            "/api/club/leave",
            json={"club_id": get_club_list["data"][0]["id"]},
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
        self.assertTrue(leave_club["type"] == "success")

        get_club_list_2 = self.client.get(
            "/api/club/get_all",
            headers={"Authorization": "Bearer {}".format(self.token)},
        ).json
